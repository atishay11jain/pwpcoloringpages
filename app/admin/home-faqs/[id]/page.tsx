...
export default function EditHomeFAQPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    if (!isNew) {
      fetch(`/api/admin/home-faqs/${id}`)
        .then((r) => r.json())
        .then((data) => setForm(data.faq))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const url = isNew ? '/api/admin/home-faqs' : `/api/admin/home-faqs/${id}`;
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white">{isNew ? 'Add Home FAQ' : 'Edit Home FAQ'}</h1>
